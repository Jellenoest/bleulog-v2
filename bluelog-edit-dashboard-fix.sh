...
echo "1/2 Bewerken-link corrigeren..."

python3 - <<'PY'
from pathlib import Path

p = Path("app/dives/[id]/page.tsx")
s = p.read_text()

old = 'href={`/dives/${dive.id}/edit`}'
new = 'href={`/dives/edit/${dive.id}`}'

if old in s:
    s = s.replace(old, new)
    p.write_text(s)
    print("Bewerken-link aangepast naar /dives/edit/[id].")
else:
    print("LET OP: oude bewerken-link niet gevonden. Controleer app/dives/[id]/page.tsx handmatig.")
PY

echo "2/2 Dashboard laten rekenen met Supabase-duiken..."
...
echo "git add app/page.tsx 'app/dives/[id]/page.tsx'"
echo "git commit -m 'Fix edit route and Supabase dashboard'"
echo "git push"
